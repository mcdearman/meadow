...
/// What the collector has done, for `MEADOW_AOT_CYCLES`: runs, candidates
...
/// Is the collector's tally wanted? `MEADOW_AOT_CYCLES`.
pub fn counting() -> bool {
    static ON: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *ON.get_or_init(|| std::env::var_os("MEADOW_AOT_CYCLES").is_some())
}
