...
//!
//! Ported from `meadow-rts`'s, operation for operation -- the wrapping
...
//
// `MEADOW_AOT_PRIMS=1`: at exit, how many times each primitive was called
// through `meadow_prim` rather than done inline -- what to inline next.

fn counting() -> bool {
    static ON: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *ON.get_or_init(|| std::env::var_os("MEADOW_AOT_PRIMS").is_some())
}
